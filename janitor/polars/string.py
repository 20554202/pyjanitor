...
def str_tokenize(
    c: str | pl.Expr, pattern: str = r"(?u)\b\w\w+\b", stem: bool = False
) -> pl.Expr:
    """
...
def str_jaro(
    c: str | pl.Expr, other: str | pl.Expr, parallel: bool = False
) -> pl.Expr:
    """
...
def similar_to_vocab(
    ...
    if metric == "lv":
        sims = [
            str_leven(c, pl.lit(w, dtype=pl.String), return_sim=True)
            for w in vocab
        ]
    elif metric == "dlv":
        sims = [
            str_d_leven(c, pl.lit(w, dtype=pl.String), return_sim=True)
            for w in vocab
        ]
    elif metric == "osa":
        sims = [
            str_osa(c, pl.lit(w, dtype=pl.String), return_sim=True)
            for w in vocab
        ]
    elif metric == "jw":
        sims = [
            str_jw(c, pl.lit(w, dtype=pl.String), return_sim=True)
            for w in vocab
        ]
    else:
...
def normalize_string(
    c: str | pl.Expr, form: Literal["NFC", "NFKC", "NFD", "NFKD"]
) -> pl.Expr:
    """
...
def normalize_whitespace(
    c: str | pl.Expr, only_spaces: bool = False
) -> pl.Expr:
    """
